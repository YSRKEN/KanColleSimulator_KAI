...
public:
	...
	explicit SharedRand(const SharedRand& o) noexcept : generator_(o.generator_), defalut_dist_(defalut_dist_) {}
	explicit SharedRand(SharedRand&& o) noexcept : generator_(std::move(o.generator_)), defalut_dist_(defalut_dist_) {}
	...
	RandType generate(const RandType min, const RandType max) {
		if (this->is_generatable()) {
			return distribution_t<int>(min, max)(*this->generator_);
		}
		else {
			throw missing_rand_generator();
		}
