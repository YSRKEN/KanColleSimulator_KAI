...
void Simulator::AirWarPhase() {
	...
	case kAirWarStatusBest:
		killed_airs_per[kFriendSide] = RandReal(7, 15) / 256;
		killed_airs_per[kEnemySide] = RandReal();
		break;
	case kAirWarStatusGood:
		killed_airs_per[kFriendSide] = RandReal(20, 45) / 256;
		killed_airs_per[kEnemySide] = RandReal() * 0.8;
		break;
	case kAirWarStatusNormal:
		killed_airs_per[kFriendSide] = RandReal(30, 75) / 256;
		killed_airs_per[kEnemySide] = RandReal() * 0.6;
		break;
	case kAirWarStatusBad:
		killed_airs_per[kFriendSide] = RandReal(45, 105) / 256;
		killed_airs_per[kEnemySide] = RandReal() * 0.4;
		break;
	case kAirWarStatusWorst:
		killed_airs_per[kFriendSide] = RandReal(65, 150) / 256;
	...
	for (size_t i = 0; i < kBattleSize; ++i) {
		...
		for (auto &it_k : fleet_[other_side].GetUnit().front()) {
			...
			for (auto& it_w : it_k.GetWeapon()) {
				...
				//割合撃墜
				if (RandBool()) killed_airs += int(int(0.9 * all_anti_air) * it_w.GetAir() / 360);
				//対空カットイン成功時の固定ボーナス
		...
		for (size_t ui = 0; ui < friend_unit.size(); ++ui) {
			...
			for (const auto& it_w : hunter_kammusu.GetWeapon()) {
				...
				case WC("艦上攻撃機"):
					...
					base_attack = int((RandBool() ? 1.5 : 0.8) * (it_w.GetTorpedo() * sqrt(it_w.GetAir()) + 25));
					break;
				default:
					base_attack = 0;
					break;
...
vector<vector<std::pair<KammusuIndex, Range>>> Simulator::DetermineAttackOrder(FireTurn fire_turn, size_t fleet_index) const
{
	...
	for (size_t bi = 0; bi < kBattleSize; ++bi) {
		...
		if (fire_turn == kFireFirst) {
			// 一覧をシャッフルする
			std::shuffle(attack_list[bi].begin(), attack_list[bi].end(), GetRandomEngine());
			// 一覧を射程で安定ソートする
		...
		const bool has_bb = [] (const vector<Fleet>& fleet) -> bool {
			for (auto &it_b : fleet) {
				for (auto &it_k : it_b.GetUnit().front()) {
					if (it_k.AnyOf(SC("戦艦") | SC("航空戦艦") | SC("陸上型")))
						return true;
				}
			}
			return false;
...
void Simulator::NightPhase() {
	...
	for (size_t ui = 0; ui < kMaxUnitSize; ++ui) {
		for (size_t bi = 0; bi < kBattleSize; ++bi) {
			...
			// 攻撃の種類によって、基本攻撃力および倍率を算出する
			// 夜戦速吸は対潜を常に爆雷で行う
			auto base_attack = hunter_kammusu.NightAttack(fire_type, target_kammusu.AnyOf(SC("陸上型"))) + (wsn_flg ? 5 : 0);
...
int Simulator::CalcDamage(
	...
	// 装甲乱数の分だけダメージを減少させる
	damage -= 0.7 * target_kammusu.AllDefense() + 0.6 * RandInt(target_kammusu.AllDefense());
	// 弾薬量補正
	...
	if (!RandBool(hit_prob | limit(0.0, 1.0))) {
	...
	if (damage < 1.0) {
		damage = 0.06 * target_kammusu.GetHP() + 0.08 * RandInt(target_kammusu.GetHP());
	}
	return int(damage);
}
int Simulator::CalcDamage(
...
void Simulator::ProtectOracle(const size_t defense_side, KammusuIndex &defense_index) const {
	...
	// 陸上型をかばう艦などいない
	if (fleet_[defense_side].GetUnit()[defense_index.fleet_no][0].AnyOf(SC("陸上型"))) return;
	...
	if (RandBool(0.4)) {	//とりあえず4割に設定している
		defense_index.fleet_i = *select_random_in_range(std::cbegin(block_list), std::cbegin(block_list) + block_list_size);
	}
	return;
}
...
DayFireType Simulator::JudgeDayFireType(const size_t turn_player, const KammusuIndex &attack_index, const KammusuIndex &defense_index) const noexcept {
	...
	if (hunter_kammusu.AnyOf(SC("正規空母") | SC("装甲空母") | SC("軽空母"))) return kDayFireAir;
	if(hunter_kammusu.AnyOf(SC("給油艦")) && hunter_kammusu.IsFireGunPlane()) return kDayFireAir;
	// それ以外は全て砲撃
	return kDayFireGun;
}
...
tuple<bool, double> Simulator::JudgeDaySpecialAttack(const size_t turn_player, const KammusuIndex &attack_index) const {
	...
	if (cutin_gg_flg || cutin_ga_flg || cutin_gr_flg || cutin_gs_flg || double_flg) {
		...
		if (RandBool(oracle | limit(0.0, 0.99))) {
			...
			const size_t rand_pos = *select_random_in_range(std::cbegin(roulette), std::cbegin(roulette) + roulette_size);
