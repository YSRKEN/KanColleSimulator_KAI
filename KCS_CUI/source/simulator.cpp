...
tuple<AirWarStatus, vector<double>> Simulator::AirWarPhase(const bitset<kBattleSize> &search_result) {
	...
	for (auto i = 0; i < kBattleSize; ++i) {
		...
		// 触接の選択率を計算する
		all_attack_plus[i] = fleet_[i].TrailerAircraftPlus(this);
	}
	...
	for (auto i = 0; i < kBattleSize; ++i) {
		...
		// 対空カットイン判定を行う
		int aac_type = fleet_[i].AacType(this);
		// 迎撃！
		//加重対空値
		...
		for (auto &it_k : fleet_[other_side].GetUnit()[0]) {
			for (auto wi = 0; wi < it_k.GetSlots(); ++wi) {
				auto &it_w = it_k.GetWeapon()[wi];
				if (!it_w.IsAirFight()) continue;
				Kammusu &intercept_kammusu = fleet_[i].GetUnit()[0][fleet_[i].RandomKammusu(this)];	//迎撃艦
				...
				//割合撃墜
				if (RandInt(2)) killed_airs += int(int(0.9 * all_anti_air) * it_k.GetAir()[wi] / 360);
				//対空カットイン成功時の固定ボーナス
	...
	for (auto bi = 0; bi < kBattleSize; ++bi) {
		...
		for (auto ui = 0u; ui < friend_unit.size(); ++ui) {
			...
			for (auto wi = 0; wi < friend_kammusu.GetSlots(); ++wi) {
				...
				// 爆撃する対象を決定する
				auto target = fleet_[other_side].RandomKammusuNonSS(this, friend_kammusu.HasAirBomb());
				// 基礎攻撃力を算出する
				int base_attack;
				...
				case kWeaponClassPA:
					...
					base_attack = int((RandInt(2) ? 1.5 : 0.8) * (friend_weapon[wi].GetTorpedo() * sqrt(friend_kammusu.GetAir()[wi]) + 25));
					break;
				default:
					base_attack = 0;
					break;
				...
				auto damage = RandInt(0, base_attack);	//仮置きのメソッド
