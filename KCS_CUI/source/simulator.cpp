	...
	for (auto bi = 0; bi < kBattleSize; ++bi) {
		...
		for (auto ui = 0u; ui < friend_unit.size(); ++ui) {
			...
			for (auto wi = 0; wi < friend_kammusu.GetSlots(); ++wi) {
				...
				all_damage[other_side][enemy_index.fleet_no_][enemy_index.fleet_i_] += damage;
			}
		}
	}
	// ダメージ処理
...
void Simulator::TorpedoPhase(const TorpedoTurn &torpedo_turn) {
	...
	for (auto bi = 0; bi < kBattleSize; ++bi) {
		...
		for (auto ui = 0u; ui < friend_unit.size(); ++ui) {
			...
			result_.AddDamage(bi, 0, ui, damage);
			all_damage[other_side][enemy_index.fleet_i_] += damage;
		}
	}
	// ダメージ処理
	for (auto bi = 0; bi < kBattleSize; ++bi) {
...
int Simulator::CalcDamage(
	...
	const auto &hunter_kammusu = friend_side.GetUnit()[friend_index.fleet_no_][friend_index.fleet_i_];
	const auto &target_kammusu = enemy_side.GetUnit()[enemy_index.fleet_no_][enemy_index.fleet_i_];
	// 攻撃の命中率を計算する
...
void Simulator::ProtectOracle(const int &defense_side, KammusuIndex &defense_index) {
	// 旗艦ではない場合、かばいは発生しない
	if (defense_index.fleet_i_ != 0) return;
	// 陸上型をかばう艦などいない
	if (fleet_[defense_side].GetUnit()[defense_index.fleet_no_][0].GetShipClass() == kShipClassAF) return;
	// 水上艦は水上艦、潜水艦は潜水艦しかかばえないのでリストを作成する
	auto &attendants = fleet_[defense_side].GetUnit()[defense_index.fleet_no_];
	...
	if (rand.RandBool(0.4)) {	//とりあえず4割に設定している
		defense_index.fleet_i_ = block_list[rand.RandInt(block_list.size())];
	}
	return;
}
